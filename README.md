...
| 特性 | 说明 |
|------|------|
| **🤖 AI 动态风险评分** | 支持 7 种 LLM（Claude/DeepSeek/OpenAI/Gemini/Groq/llama.cpp/Ollama），PII 本地脱敏后发送，动态调整权限（允许/MFA/审批/拒绝），降级回退确定性规则评分 |
...
![系统架构.drawio (1)](./images/SystemArchitecture.png)
...
| **Risk Service** | 8090 | HTTP/HTTPS | AI 风险评分：Claude/DeepSeek/OpenAI/llama.cpp/Ollama 等 LLM 多维度评估，支持缓存和降级 |
...
FileGuard 支持接入本地部署的大语言模型，无需云端 API Key，数据不出内网。

#### 方式 A：llama.cpp

[llama.cpp](https://github.com/ggerganov/llama.cpp) 是一个高性能 C++ 推理引擎，支持 GGUF 格式模型。

```bash
# 1. 启动 llama-server（GitHub 下载 llama.cpp 或自行编译）
llama-server -m ./Qwen3.5-9B-Q6_K.gguf \
  --ctx-size 16384 \
  --port 8079 \
  --n-gpu-layers 35

# llama-server 启动后自动监听 /v1/chat/completions（OpenAI 兼容 API）
```

```yaml
# 2. 配置 configs/riskservice.yaml
llm:
  provider: llamacpp
  model: Qwen3.5-9B-Q6_K               # 任意标识，透传给 llama-server
  endpoint: http://127.0.0.1:8079/v1/chat/completions
  api_key_env: FILEGUARD_LLM_API_KEY    # 不会实际校验，可不设置
  timeout: 30s                          # 本地推理较慢，建议 >= 30s
  max_retries: 1
```

```bash
# 3. 启动 Risk Service（无需设置 API Key）
go run ./cmd/riskservice -config configs/riskservice.yaml
```

#### 方式 B：Ollama

[Ollama](https://ollama.com/) 是最流行的本地 LLM 运行工具，默认监听 `11434` 端口。
...
| 维度 | 云端 LLM（Claude/DeepSeek/OpenAI） | 本地部署（llamacpp/Ollama） |
|------|-----------------------------------|---------------------------|
| **API Key** | 必须设置 `FILEGUARD_LLM_API_KEY` | **不需要**（`RequiresAPIKey() → false`） |
| **网络** | 需要出网访问 | 纯内网，数据不出境 |
| **延迟** | 100-500ms | 取决于硬件（GPU 推理通常 1-5s） |
| **timeout 建议** | 10-15s | **30s+** |
| **模型** | 云端最新版本 | 自行选择 GGUF/GGML 模型 |
| **成本** | 按 token 计费 | 仅硬件 + 电费 |
| **合规** | 数据发送至第三方 | **数据完全本地化** |

> **原理**：llama.cpp server 和 Ollama 都暴露标准 OpenAI `/v1/chat/completions` 格式 API，FileGuard 复用 `OpenAICompatibleProvider`，通过 `noAuth: true` 跳过 API Key 校验。`RequiresAPIKey()` 接口方法确保本地 Provider 返回 `false`，Scorer 不会因缺少 Key 而直接降级。
...
llm:
  provider: deepseek         # anthropic | openai | deepseek | google | groq | llamacpp | ollama
...
- **AI 模型**：支持 Anthropic Claude、OpenAI、DeepSeek、Google Gemini、Groq、llama.cpp、Ollama 七种 LLM 提供商，可扩展自定义 Provider
